...
<body>
    ...
    <form method="POST">
        <div class="container col2 mt-4" style="text-align:center;">
            <p class="fs-1 fw-bold info"> Hall Of Books </p>
        </div>
        <div class="container align-center">
            <div class="card-header " style="background-color: #404e4d;">
                <ul class="nav nav-tabs card-header-tabs">
                    <?php
                    $select = "SELECT * FROM  `subcat` WHERE type = 'books' ";
                    $q = mysqli_query($conn, $select);
                    foreach ($q as $data) {
                    ?>
                        <li class="nav-item">
                            <a href="./books.php?cat=<?php echo $data['id']; ?>" name="category" class="nav-link text-light"><?php echo $data['subname'] ?></a>
                        </li>
                    <?php } ?>
                </ul>
            </div>
                    ...
                    ?>
                        <div class="card m-5">
                            <div>
                                ...
                                <div class="card-body">
                                    <h5 class="card-title"> Book Name : <?php echo $data['proName'] ?> </h5>
                                    <hr>
                                    <p class="card-text"> Sammary : " <?php echo $data['description'] ?> "</p>
